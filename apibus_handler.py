...
class SaeApibusAuthHandler(BaseHandler):
    ...
    def http_request(self, req):
        ...
        resource = urllib.parse.unquote(req.get_full_url()[len(_APIBUS_URL_PREFIX)-1:])
        sae_headers = [(k.lower(), v.lower()) for k, v in list(req.headers.items()) if k.lower().startswith('x-sae-')]
...
try:
    ...
    class SaeApibusAuth(AuthBase):
        ...
        def __call__(self, r):
            ...
            resource = urllib.parse.unquote(r.url[len(_APIBUS_URL_PREFIX)-1:])
            #resource = r.url[len(_APIBUS_URL_PREFIX)-1:]
            sae_headers = [(k.lower(), v.lower()) for k, v in list(r.headers.items()) if k.lower().startswith('x-sae-')]
...
def _signature(secret, method, resource, headers):
    ...
    msgToSign = bytes(msgToSign, 'latin-1')
    return bytes("SAEV1_HMAC_SHA256 ", 'latin-1') + base64.b64encode(hmac.new(secret, msgToSign, hashlib.sha256).digest())